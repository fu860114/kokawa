...
class CaptchaNumbers {
	var $length = 6;
	var $font = 'include/captcha/arial.ttf';
	var $size = 15;
	var $type = 'jpeg';
	var $height = 40;
	var $width = 80;
	...
	*/		
	function generate() {
		...
		for($i = 0; $i < strlen($this -> getString()); $i++){

			$strpos = rand(1,8) - 4;
			ImageTTFText($image, $this -> size, 0, $this -> grid + $i * ($this -> size - 2), $this -> size + $this -> grid + $strpos, $stringcolor, $this -> font, substr($this -> getString(),$i,1));
