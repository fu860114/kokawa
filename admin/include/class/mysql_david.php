<?php
	...
	class Mysql {
		...
		// function 內部自己 呼叫用  如 create()update() recover()
		...
		// david 自改的 query 參考 _query() 改寫
		function query_david($queryString, $valary = null)
		{  // 20220120 david 修改 改傳回 1.執行成功 -1.執行失敗 在除錯模式下,顯示錯誤訊息
 			if ( $valary == null ) {
                return -2 ;  // 因 pdo->query($queryString) 會 造成 rowCount() 發生錯誤 , 所以這方法不採用
				// $stmt = $this->pdo->query($queryString);  // 直接執行sql command 
			}
			//
			
			   $stmt = $this->pdo->prepare( $queryString );  // 自動帶入 對應參數
               $stmt->execute( $valary );  
               $ln_dbrwcc = $stmt->rowCount(); // 取出 影嚮的筆數
			 
			//
            // echo "<b>ln_dbrwcc:</b> $ln_dbrwcc <br>\n";
			$errormsg = $this->pdo->errorInfo();
			if ( $errormsg[0] != "00000" ) { 
			    // $this->halt("Invalid SQL: " .$errormsg[2] ); 
			    if (DebugMode) {  // 除錯模式
				   echo "<b>Database error_code:</b> $errormsg[0] <br>\n";
				   echo "<b>Database error_mssage:</b> $errormsg[2] <br>\n";
				}
		...
		function &getRow($queryString, $valary = null)
		{
            ...
            // david 202220120 把上面的改成如下 , 增加 判斷 陣列size後,再放入回傳的值
			if (!is_array($getdata)) {
				$results = null;
            } else { 
			      if( sizeof($getdata)> 0 ) { // 有找到資料
				      //echo "sizeof(getdata):".sizeof($getdata)."<BR>" ;
					  // die() ;
			          $results = $getdata[0]; 
				  } else {
			          $results = null ; // 沒有找到資料
