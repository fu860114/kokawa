 <?php
	...
	//初始值設定
	$_Table_Main   = "partner";		    //本檔案主資料庫
	$_File_Name    = basename(__FILE__);          //目前檔名

	$twig->addGlobal('function_title', "合作夥伴");
	$twig->addGlobal('document_path', "合作夥伴");
	...
	if ($_REQUEST['flag'] == 'edit') {
		...
		$template = $twig->load('partneredit');
		$displayary = array();
	...
	elseif ($_REQUEST['flag'] == 'editsave') {
		...
		if ($_FILES["iconpic"]["name"] != "") {
			$data['iconpic'] = "product-" . date("Ymdhis") . substr($_FILES["iconpic"]["name"], strrpos($_FILES["iconpic"]["name"], '.'));
	...
	/*------------------------------------------------------------------------------------------------------------*/ else {
		...
		$template = $twig->load('partnerlist');
		$displayary = array();
		echo $template->render($displayary);
	}
	?>